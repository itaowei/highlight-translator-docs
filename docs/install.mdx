...
![Edge 扩展商店版本](https://img.shields.io/static/v1?label=版本&message=v8.6.0&color=informational&style=flat-square)
...
<details>
  <summary>为什么 Firefox 的版本偶尔不可见或不是最新版本？</summary>
  Firefox 对扩展的审核非常严格，但他们又非常不仔细，只要他们发现了
  <strong>疑似</strong>
  有问题的代码（但实际上根本就不是他们认为的那样）就会将划词翻译下架（版本号不可见的情况）或者将划词翻译降级至他们认为没有问题的版本（版本号不是最新版本的情况）。而且，Firefox
  的审核团队效率很低，一旦被标记为需要人工审核，就会出现过了两三个星期都还没开始重审的情况。由于
  Firefox 不提供离线安装包，所以我能做的只有催促他们尽快完成重审。
</details>
