...
划词翻译只会将你划选的文本这一个信息发送给翻译服务器。除非你主动开启了[历史记录](guides/history.md)或将翻译结果加入了[收藏夹](guides/fav.mdx)，否则划词翻译不会保存此文本的关联信息，例如你划选这段文本的网址、这段文本的上下文等。除非你启用了数据同步，否则历史记录和收藏夹只会保存在你的浏览器中，不会上传到划词翻译的服务器。
