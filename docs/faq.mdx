...
我 2013 年发布了划词翻译（[查看划词翻译大事记](histroy.mdx)），在 2020 年发布 v7.0 版本之前，划词翻译的代码是在[全球最大的程序员网站 GitHub 上开源的](https://github.com/lmk123/crx-selection-translate)。也正因为如此，在划词翻译 35 万的用户当中，有很大一部分是程序员，即使 v7.0 版本之后没有开源了，但如果划词翻译做出了侵犯隐私的事情，程序员也有办法发现。

### 划词翻译发送的信息 {#safe-data}

划词翻译只会将你划选的文本这一个信息发送给翻译服务器。除非你主动开启了[历史记录](guides/history.mdx)或将翻译结果加入了[收藏夹](guides/fav.mdx)，否则划词翻译不会保存此文本的关联信息，例如你划选这段文本的网址、这段文本的上下文等。除非你启用了数据同步，否则历史记录和收藏夹只会保存在你的浏览器中，不会上传到划词翻译的服务器。

### 你填写的密钥不会上传 {#safe-key}

你申请的密钥只会保存在浏览器中，即使开启了数据同步也不会上传。虽然这样一来，你每在一个新的浏览器使用划词翻译时都要重新填写一遍密钥，但我认为为了确保你的密钥安全，这是值得的。

<details>
  <summary>不上传密钥的具体原因</summary>
  <p>如果我为了同步这些密钥而将它们上传到划词翻译的服务器中：</p>
  <ol>
    <li>如果有人怀疑我盗用了这些密钥，我是没法自证清白的。</li>
    <li>我没有 100% 的把握保证划词翻译的服务器不会被入侵。</li>
  </ol>
  虽然可以使用本地加解密的手段将密文保存到服务器，但这样一来就需要用户在本地设置一个二级密码，进一步增加了复杂度，但实际上常用的浏览器一般不会超过
  4 个，所以重新填写这些密钥虽然麻烦但也只麻烦 4 次。综合来看，不上传密钥是最好的选择。
</details>

### 自动读取剪切板是可关闭的 {#safe-clipboard}

划词翻译默认开启了【自动读取剪切板】功能，这是为了方便你打开[独立翻译窗口](guides/standalone.mdx)和[扩展弹出页](guides/popup.mdx)时自动读取剪切板并翻译，省去粘贴的操作。这个功能是可以在[设置页](guides/options.mdx)里关闭的。

## 为什么翻译服务需要购买翻译积分或申请密钥才能使用？ {#why-apply}

这些翻译服务之所以需要申请密钥，是因为它们的官方渠道就是要通过密钥使用的。以百度翻译为例，百度翻译提供了两种使用渠道：

1. 网页在线版 [https://fanyi.baidu.com/](https://fanyi.baidu.com/)、百度翻译 APP 等这种由百度翻译官方出品的形式
2. 翻译 API [https://fanyi-api.baidu.com/](https://fanyi-api.baidu.com/)

网页在线版 / APP 虽然是免费的，但它们是由百度官方提供的，仅限于通过网页访问或安装了 APP 的终端用户使用。划词翻译作为一款不是由百度翻译官方出品的第三方软件，只能通过翻译 API 的形式使用，而翻译 API 的形式就需要在它们的平台里申请密钥。

虽然划词翻译可以把自己的密钥提供给所有用户使用（这样一来所有用户都不用申请密钥就可以直接使用了），但划词翻译的用户太多了而翻译平台提供的免费额度又是有限的，超过免费额度之后就要收费，所以最好的办法就是你自己去申请密钥，独享翻译平台提供的免费额度。

如果你实在不想自己申请密钥，那么你可以购买[翻译积分](services/tp.mdx)来使用这些服务，这其实就是通过划词翻译自己的密钥来访问这些服务，而你购买的翻译积分就是用来支付这些翻译平台的翻译字符费用了，划词翻译仅赚取极少的差价用来维持服务器~~和我~~的运转。

## 如何翻译 PDF 文件？ {#how-pdf}

请查看[翻译 PDF 文件](guides/pdf.mdx)。
...
如果你是 Firefox 浏览器，由于 Firefox 不提供本地文件的权限，所以无法做到在本地文件里使用，但你可以在复制文本后，点击地址栏右侧的划词翻译图标（或使用快捷键打开）使用。你还可以使用[独立翻译窗口](guides/standalone.mdx)。
...
### 如果谷歌翻译出现了一直加载不显示结果的问题 {#google-only}

首先，请确认你没有[修改划词翻译的权限范围](guides/options.mdx#per)。

确认没有修改后，请开梯子之后重试。原因见[谷歌翻译的备用接口现在需要开梯子才能使用了](/blog/2021/09/22/google-force-com)。

### 如果 DeepL 出现了一直加载不显示结果的问题 {#deepl-only}
...
- 各浏览器自己的一些其它限制，目前已知的有 Firefox 不允许在 [https://support.mozilla.org/](https://support.mozilla.org/) 里使用。

在这些页面中，唯一可以使用的方式是划选文本后，用鼠标右键点击你选中的文本，然后选择【翻译网页中选中的文本】，之后划词翻译会弹出[独立翻译窗口](guides/standalone.mdx)并翻译。
...
- [Tesseract OCR](https://tesseract-ocr.github.io/)：用于在[截图 / 图片翻译功能](guides/screenshot.mdx)中将图片转为文字。
- [PDF.js](https://mozilla.github.io/pdf.js/)：用于[翻译 PDF 文件](guides/pdf.mdx)。
