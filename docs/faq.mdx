...
虽然可以使用本地加解密的手段将密文保存到服务器，但这样一来就需要用户在本地设置一个二级密码，进一步增加了复杂度，但实际上常用的浏览器一般不会超过 4 个，所以重新填写这些密钥虽然麻烦但也只麻烦 4 次。综合来看，不上传密钥是最好的选择。
</details>
...
如果你实在不想自己申请密钥，那么你可以购买[翻译积分](services/tp.md)来使用这些服务，这其实就是通过划词翻译自己的密钥来访问这些服务，而你购买的翻译积分就是用来支付这些翻译平台的翻译字符费用了，划词翻译仅赚取极少的差价用来维持服务器~~和我~~的运转。
